  ...
  testList "CustomerBasket" [
    testList "AddItem" [
      testCase "Add item when basket is empty" <| fun _ ->
        ...
        let subject = CustomerBasket.zero(Guid.NewGuid())(userId) |> CustomerBasket.addItem { Id = uuid; Quantity = 1 }
      ...
      testCase "Add item when basket is not empty" <| fun _ ->
        ...
        let userId = Guid.NewGuid()
        let subject =
          CustomerBasket.zero(Guid.NewGuid())(userId) |> CustomerBasket.addItem { Id = uuid; Quantity = 1 }  |> CustomerBasket.addItem { Id = uuid; Quantity = 1 }
      ...
      testCase "Add two distinct items when basket is not empty" <| fun _ ->
        ...
        let userId = Guid.NewGuid()
        let subject = CustomerBasket.zero(Guid.NewGuid())(userId) |> CustomerBasket.addItem { Id = uuid; Quantity = 1 }  |> CustomerBasket.addItem { Id = uuid2; Quantity = 1 }
    ...
    testList "RemoveItem" [
      testCase "Remove item when basket is empty" <| fun _ ->
        let uuid = Guid.NewGuid()
        let userId = Guid.NewGuid()
        let subject = CustomerBasket.zero(Guid.NewGuid())(userId) |> CustomerBasket.removeItem { Id = uuid; Quantity = 1 }
      ...
      testCase "Remove item when basket is not empty ad item not exists" <| fun _ ->
        ...
        let userId = Guid.NewGuid()
        let subject =
          CustomerBasket.zero(Guid.NewGuid())(userId)
      ...
      testCase "Remove item when basket is not empty ad item exists" <| fun _ ->
        ...
        let uuid2 = Guid.NewGuid()
        let userId = Guid.NewGuid()
        let subject = CustomerBasket.zero(Guid.NewGuid())(userId)
                        |> CustomerBasket.addItem { Id = uuid; Quantity = 1 }
      ...
      testCase "Remove items from basket with possible quantity" <| fun _ ->
        ...
        let uuid2 = Guid.NewGuid()
        let userId = Guid.NewGuid()
        let subject = CustomerBasket.zero(Guid.NewGuid())(userId) |> CustomerBasket.addItem { Id = uuid; Quantity = 1 }  |> CustomerBasket.addItem { Id = uuid2; Quantity = 1 } |> CustomerBasket.removeItem { Id = uuid; Quantity = 1 } |> CustomerBasket.removeItem { Id = uuid2; Quantity = 1 }
