...
[<Pojo>]
type ProductsQuery = { page: int; pageSize: int; sort: string; filters: ProductFilter array }
...
let addFiltersToQueryString (filters: ProductFilter list) url =
  let rec func (f: ProductFilter list) (acc: string) =
    ...
    | head::tail ->
      match head with
      | PriceMin p ->
        func tail (acc + (sprintf "&priceMin=%.2f" p))
      | PriceMax p ->
        func tail (acc + (sprintf "&priceMax=%.2f" p))
      | Name name ->
        func tail (acc + (sprintf "&name=%s" name))
  url + (func filters "")

let getProducts(query: ProductsQuery) =
  promise {
    let url = sprintf "%s%s" Urls.CatalogApiUrl (sprintf "products?pageSize=%d&pageIndex=%d&sort=%s" query.pageSize query.page query.sort) |> addFiltersToQueryString (query.filters |> Array.toList)
    let props =
...
let init () : Model * Cmd<Msg> =
  { Products = [||]; Page = 1; PageSize = 15; TotalItems = 0; TotalPages = 0; ErrorMessage = None; Loading = false; Sort = "default" }, getProductsCmd { page = 1; pageSize = 15; sort = "priceAsc"; filters = [| PriceMin(15m) |] }

let update msg model =
  match msg with
  | BrowseProducts(page, pageSize, sort, filters)->
    let query: ProductsQuery = { page = page; pageSize = pageSize; sort = sort; filters = filters  }
